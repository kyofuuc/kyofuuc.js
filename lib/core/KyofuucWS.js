...
function KyofuucWS(wsConfig) {
	...
	this.httpInterceptor = this.wsConfig.interceptor || new FuInterceptor();
	this.eventHooks = {
		"onOpen": [],
		"onMessage": [],
		"onClose": [],
...
KyofuucWS.prototype.ws = function ws(urlOrConfig, config) {
	const instance = utils.cloneInstance(this);
	...
	config = utils.mergeObject(config, instance.httpBaseConfig);
	const connector = (config.connector || defaults.getDefaultWSConnector());
	instance.connection = connector(config, instance.httpInterceptor);
	instance.connectionImplName = instance.connection.implname;

	// onError event
	instance.httpInterceptor.registerOnWSError((options, config, event) => {
		utils.forEach(instance.eventHooks.onError, (cb) => cb(instance, event));
	}, { instance });

	// onClose event
	instance.httpInterceptor.registerOnWSClose((options, config, event) => {
		instance.state = event.target.readyState
		utils.forEach(instance.eventHooks.onClose, (cb) => cb(instance, event));
	}, { instance });

	// onOpen event
	instance.httpInterceptor.registerOnWSOpen((options, config, event) => {
	...
	// onMessage event
	instance.httpInterceptor.registerOnWSMessage((options, config, event) => {
		let data;
		let sanitizedData;
