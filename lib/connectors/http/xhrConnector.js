...
function xhrConnector(config, httpInterceptor) {
	return new Promise(function makeRequest(resolvePromise, rejectPromise) {
		...
		if (utils.isFormData(reqData) && reqHeaders['Content-Type']) {
			delete reqHeaders['Content-Type'];
		} else if ((utils.isPlainObject(reqData) || utils.isObject(reqData) || utils.isArray(reqData)) && !reqHeaders['Content-Type']) {
