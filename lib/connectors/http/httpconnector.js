			...
			let req = transport.request(options, function handleResponse(res) {
				...
				if (config.responseType === 'stream') {
					...
					stream.on('error', function handleStreamError(err) {
						if (req.aborted) return;
						reject(utils.kyofuucError(err, config, utils.ERROR_CODES.STREAM_ERROR, lastRequest));
					});
					...
					stream.on('end', function handleStreamEnd() {
						try {
							...
							reject(utils.kyofuucError(err, config, utils.ERROR_CODES.ERR_PROCESSING_DATA, response.request, response));
						}
						utils.resolveResponse(response, resolve, reject);
					});
				}
