...
function httpConnector(config, httpInterceptor) {
	return new Promise(function makeRequest(resolvePromise, rejectPromise) {
		...
		let resolve = function resolve(value) {
			if (rejected) return;
			utils.invokeForEachInterceptorType(httpInterceptor, "POST_RESPONSE", config, value);
		...
		function processResponse(res, lastRequest, onSuccess, onError) {
			let stream = res;
			let response = {
		...
		function transportRequest(redirectCount) {
			let req = transport.request(options, function handleResponse(res) {
				...
				if ((res.statusCode === 301 || res.statusCode === 302) && res.headers['location']) {
					...
					if ((++redirectCount) <= config.maxRedirects) {
						...
						if (config.storeRedirectsData) {
							...
							processResponse(res, lastRequest, resolveReResponse, resolveReResponse);
						} else {
							req.abort();
						}
				...
				processResponse(res, lastRequest,function onSuccess(result) {
					if (config.storeRedirectsData) {
